...
class mapcol_departamentos:
    ...
    @staticmethod
    def figura(self):
        with open('assets/data/colombia.geo.json', encoding='utf-8') as json_file:
            departamentos = json.load(json_file)
        for i, each in enumerate(departamentos["features"]):
            departamentos["features"][i]['id']=departamentos["features"][i]['properties']['NOMBRE_DPT']

        mapa = go.Choroplethmapbox(
            geojson=departamentos, 
            locations=self.df.DEPARTAMENTO, 
            z=self.df['CANTIDAD'],
            colorscale="dense",
        ...
        fig.update_layout(
            ...
            mapbox_zoom=3.9, 
            mapbox_center = {"lat": 4.6269447, "lon": -74.0664136},
